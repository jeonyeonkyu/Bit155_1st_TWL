...
public class BoardDao {
	...
	public int deleteOk(String idx , String pwd) {
		...
		원본글 (답글이 있는 경우)
		case 1: 원본글이 삭제시 답변글 까지 다 삭제 (같은 refer delete)
	...
	public List<Reply> replylist(String idx_fk){
			...
			try {
				conn = ds.getConnection();
				String reply_sql = "select * from reply where idx_fk=? order by no desc"; //원본글의 idx값을 댓글테이블에서 참조 나중에 쓴 댓글이 위로 올라감
	...
	public int reWriteOk(Board boardata) {
		...
		try {
			...
			//1. 답글 
			//현재 내가 읽은 글의 refer , depth , step (원본글) 근데 이게 답글이 될 수도 있음 답글을 읽고 거기에 답글을 달 수도 있으니까
			String refer_depth_step_sal ="select refer , depth , step from jspboard where idx=?";
			
			//2. 위치
			//step (순서) : 나중에 쓴 답글이 위로 올라오게 하겠다(최신으로)
			...
			if(rs.next()) { //데이터가 있다면... 원본글의 refer, depth, step 값이 있다면
				int refer = rs.getInt("refer");
				int step = rs.getInt("step");
				int depth = rs.getInt("depth");
				
				pstmt = conn.prepareStatement(step_update_sql); //재사용이 안되어서 다시 컴파일해야함.
				//기존  step에 +1하는 update구문이 실행된 것
				pstmt.setInt(1, step);
				pstmt.setInt(2, refer);
				pstmt.executeUpdate();

				//filename,filesize,refer,depth,step
				pstmt = conn.prepareStatement(rewrite_sql); //여기도 역시 쿼리문 다시 컴파일 해야함.
				pstmt.setString(1, writer);
