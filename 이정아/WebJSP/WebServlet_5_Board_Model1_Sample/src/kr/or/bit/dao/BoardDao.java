...
public class BoardDao {
	...
	public int writeok(Board boarddata) {
		// insert into
		// jspboard(idx,writer,pwd,subject,content,email,homepage,writedate,readnum,filename,filesize,refer)
	...
	public List<Board> list(int cpage, int pagesize) {
		...
		//pagesize = 나누고 싶은 페이지 사이즈(단위) 5라면 5개씩 묶기, 10하면 10개씩 묶기
	   ...
	   "			 ) where rn >= ?"; //start row
			...
			//cpage >> 1
			//pagesize >> 5
			 int start = cpage * pagesize - (pagesize - 1); //스타트값 추출 1 * 5 - (5-1) >> 1
			 int end = cpage * pagesize; //엔드값(endlow) 추출 5
			...
			while(rs.next()) {
				...
				list.add(board); //데이터가 들어감(페이징 한 수만큼)
			}
		 ...
		 * 
		 * [1] page 크기 > pagesize 정의 totaldata > 54건 pagesize = 5 //한 페이지당 들어있는 게시글의 개수
		 * = 전체 페이지가 11페이지가 나온다. 규칙 > totalpagecount=11 (전체 페이지 개수)
		 * 
		 * int cpage >> currentpage(현재 페이지 번호) >> 1page ,2page (출력하고 싶은 페이지의 번호)
		 * 
	...
	// 게시물 총 건수를 알아야 한다. 그래야 몇개씩 묶을지 알 수 있기 때문
	public int totalBoardCount() {
		...
		try {
			conn = ds.getConnection();
			String sql = "select count(*) cnt from jspboard"; // 게시글이 총 몇건인지 확인 cnt 는 별명 준 것
			pstmt = conn.prepareStatement(sql);
			rs = pstmt.executeQuery();
			if (rs.next()) {
				totalcount = rs.getInt("cnt"); // 별명 or 1 들어 갈 수 있음
