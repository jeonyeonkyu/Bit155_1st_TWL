...
public class BoardDao {
	...
	public int deleteOk(String idx, String pwd) {
		 ...
		 * case 2:
		 * (네이버)원본글만 삭제 -> 나머지 처리 (텍스트 형태 (원본글 삭제 표시) (step, depth) 
		 * case 3: 삭제시 삭제되었을 표시 (쌤피셜 이게 괜찮은듯)
		 * ( 게시판 설계 (delok :삭제여부 컬럼 :1) >> 삭제 : 0 >> update .. 0 
		 * case 4: 삭제 못하게 한다 (답글이
		 * 있으면) refer count > 1
		 */
		Connection conn = null;
		PreparedStatement pstmt = null;
	...
	public int reWriteOk(Board boardata) {
		...
		try {
			...
			/*
			  ...
			  2.답글에 대한 순서 : 나중에 쓴 답글이 위로 올라가게 함(최신글 위로) -> update가 들어가야함
