...
class ScoreEvaluation { 
   ...
   public static void main(String args[]) { //여기가 main
      while(true) { 
         switch(displayMenu()) { 
            case 1 : 
               inputRecord(); 
      ...
      while(true) { 
         ...
         do { 
            try { 
               ...
               if(!input.equalsIgnoreCase("q")) {  //input값의 대소문자 구분없이 q가 아닐때, if문 실행
                  int length = record.size();    //record(List)의 사이즈의를 가져 옴 
                  boolean found = false;
...
class TotalDescending implements Comparator { 
   public int compare(Object o1, Object o2){ 
      if(o1 instanceof Student2 && o2 instanceof Student2){ 
         ...
         //s2의 total값이 크다가 true일 때 1, false을 때 s1과s2값 비교 후 같으면 0, 다르면 -1 리턴
      } 
      return -1; 
   } 
}
