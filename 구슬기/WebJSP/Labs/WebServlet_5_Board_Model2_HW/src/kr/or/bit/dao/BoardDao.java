...
public class BoardDao {
	...
	public int deleteOk(String idx , String pwd) {
		...
		원본글 (답글이 있는 경우)
		case 1: 원본글 삭제시 답변글 있으면 다 삭제 (같은 refer delete)
		case 2: (네이버)원본글만 삭제 -> 나머지 처리 (텍스트 형태 (원본글 삭제 표시) (step, depth)
		case 3: 삭제시 삭제되었음을 표시 ( 게시판 설계 (delok :삭제여부 컬럼 :1) >> 삭제 : 0 >> update .. 0   >>0이면 삭제니까 뿌리지않도록 if로 할수있음 
		case 4: 삭제 못하게 한다 (답글이 있으면) refer count > 1
		
		어떤케이스를 택하느냐에 따라 로직이 많이 달라진다 
		*/
		Connection conn = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
	...
	public List<Reply> replylist(String idx_fk){
			...
			try {
				conn = ds.getConnection();
				String reply_sql = "select * from reply where idx_fk=? order by no desc";//참조키가 ? 거에 대한 모든 댓글을 가져오겠다  나중에 쓴 댓글이 위로 올라오게 
	...
	public int replyDelete(String no , String pwd) {
			...
			try {
				...
				rs =pstmt.executeQuery();
				if(rs.next()) {//셀렉트 먼저 해서 결과가 있다면 패스워드받고 delete 한다 
	...
	public int reWriteOk(Board boardata) {
		...
		try {
			...
			/*
			  	답글
			  	1. 원본글(현재 read 한글) :refer, depth, step >> select
			  	
			  	2. 답글 (순서) 나중에 쓴 답글이 위로 올라오도록(최신) ...  
			  	update jspboard set step= step+1 where step  > 0 and refer =1 이런식으로...
			  	
			  	3. 답글 insert
			  	새로운게 들어가기전에 이전애들의 스텝에 +1해준다 그후 인서트 해준다 
			...
			if(rs.next()) { //원본글에 refer, step, depth가 존재하면 얘를 돈다 
				int refer = rs.getInt("refer");
				int step = rs.getInt("step");
				int depth = rs.getInt("depth");
				
				pstmt = conn.prepareStatement(step_update_sql);//다시 컴파일 시킨다 
				//기존 step +1 하는 업데이트구문 실행한다  (새로운애 넣기위한 과정)
				pstmt.setInt(1, step);
				pstmt.setInt(2, refer);
				pstmt.executeUpdate();

				//filename,filesize,refer,depth,step
				pstmt = conn.prepareStatement(rewrite_sql); //또 컴파일 시킨다 
				pstmt.setString(1, writer);
				...
				pstmt.setInt(8, refer);
				pstmt.setInt(9, depth+1); // 규칙 현재 읽은 글에 depth + 1
				pstmt.setInt(10, step+1); // 순서 update 통해서  자리 확보(해놨으니까 요렇게 넣으면 얘가 최신,위로 올라옴 ) + 1
	...
	//게시글 수정하기 처리                    //비밀번호 확인 업데이트 후 처리 
	//public int boardEdit(Board boarddata){}
	public int boardEdit(HttpServletRequest boarddata) { //여기서 또 HttpServletRequest 이거로 받음 
