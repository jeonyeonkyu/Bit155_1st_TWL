...
<body>
	...
	//옵션
	//조회수 증가  //조회수따라 어떤 이벤트를 줄수 도 있음 //예외상황이 많아서 좀 어렵다
	boolean isread = service.addReadNum(idx);
	if (isread)
		System.out.println("조회증가:" + isread);

	//데이터  조회 (1건 ..로우 1개) 글번호는 PK 키니까 한건을 보장한다 
			...
			<center>
				...
				<table width="80%" border="1">
					...
					<tr height="100">
						...
						<td colspan="3">
							<!-- 글쓸때 enter를 치면 캐리지 리턴 라인 피드값이라고 하는데 \n 이다. 엔터를 브라우저가 인식하는 <br>태그로 바꿔줘야함 -->
							<%
					...
					<tr>
						<td colspan="4" align="center"><a
							href="board_list.jsp?cp=<%=cpage%>&ps=<%=pagesize%>">목록가기</a>
						<!-- 3번페이지의 사이즈5 를 기억해서 간다  --> |<a
							href="board_edit.jsp?idx=<%=idx%>&cp=<%=cpage%>&ps=<%=pagesize%>">편집</a>
						<!-- 수정하면 기존의 썻던글이 보이고, 인서트랑 비슷한데 뿌려진화면에서 작업.  --> |<a
							...
							<!-- 원본글의 정보들을가져가야하니까.. 그리고 어떤글의 답 글인지 알기위해--></td>
					</tr>
				</table>
				...
				<form name="reply" action="board_replyok.jsp" method="POST">
					...
					<input type="hidden" name="userid" value="">   <!--  추후필요에 따라 추가 -->
					<!-- hidden data -->
					<table width="80%" border="1">
						<tr>
						...
						<tr>
							<td align="left">작성자 : 
							<input type="text" name="reply_writer"><br /> 내&nbsp;&nbsp;용 : 
				...
				<script type="text/javascript">
					function reply_check() { //내용, 작성자, 비밀번호 셋중하나 비워놨다면 false 탄다 
				...
				<%
					...
					if(replylist != null && replylist.size() > 0){ //이러면 데이터가 있는거 
