...
@WebServlet("*.do") // 모든 쩜두를 다 실행한다     하나의 컨트롤러가 여러개의 요청을 받는 방법이다 
public class RegisterFrontController extends HttpServlet {
	...
	private void doProcess(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String viewpage="";
		if(command.equals("register")) { //?cmd=register //회원가입 하려고 하는구나~
			//회원가입 페이지 전달
			viewpage = "/WEB-INF/Register/Register.jsp"; //WEB-INF 접근 가능
		}else if(command.equals("registerok")) { //?cmd=registerok //회원가입처리 구나~
			//회원가입 처리
		...
		//5. forward 
			dis.forward(request, response); //redirect 하면 주소가 바뀌어서 forward로 했다 
	}
