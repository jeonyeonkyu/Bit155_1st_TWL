...
/*  
	...
	//1. 결과 없는 경우 , 2. single row , 3. multi row 모두 만족하는 코드    //공식같은로직 아래 67 번의 코드보다는 이걸 더 추천하심 
	if(rs.next()) {
			 do {
...
*/
	...
	try{
		//싱글톤 헬퍼 사용해서  58번줄 안써도 됨  
		//conn = DriverManager.getConnection("jdbc:oracle:thin:@localhost:1521:XE","bituser","1004");
		//비기능적 요구사항
		conn = Singleton_Helper.getConnection("oracle");
		        //싱글톤 헬퍼 사용해서 하는거!
		String sql="select id, pwd from koreamember where id=?";
		pstmt = conn.prepareStatement(sql); //sql 먼저 만들어서 컴파일 시켜놓고, ? 에다가 값을 준당
		pstmt.setString(1,id);
		rs = pstmt.executeQuery(); 
		
		//업무   
		while(rs.next()){ 
			//while문을 탄다? 데이터가 있다 (id가 존재)
			...
			if(pwd.equals(rs.getString("pwd"))){
			  ...
			  //Top.jsp 정보 로그인 상태 ... 구슬기 님 반갑습니다 이런거. 
