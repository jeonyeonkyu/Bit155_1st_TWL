...
<head>
	...
	<title>CommentLIST     여러개의 목록을 나열하고 지우는 방법 </title>
	...
	<script type="text/javascript">
		$(function(){
			...
			$('#container').on("click","button",function(){
				...
				var data = {
					      bbsSeq  : "<%= bbsSeq %>" ,
					      seq : $(this).attr("seq")     //delete * from board where id=100    //버튼이 가지고 있는 attr 속성 
					
			            };
				// $.getJSON().. TEST 
				$.getJSON('CommentDel.jsp',data) //요청보내는 주소, 파라메터값
						 ...
						 $('#container').find("table tr").not(":first").remove();
							...
							$.each(deldata, function(){
								...
								$('#container').find("table tr:last").after(
									...
									+ " <td>" + this.seq + "</td>"
									+ " <td>" + this.comment + "</td>"
									+ " <td><button seq='" + this.seq +"'>삭제</button></td>" //방법은 많지만 수업대문에 꼬아놓기도 하심 
									+ "</tr>"
								);
							});
							$('#comment').val();
