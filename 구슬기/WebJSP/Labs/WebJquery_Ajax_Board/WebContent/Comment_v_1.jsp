...
<head>
	...
	<title>CommentLIST            보낼때 제이슨으로 보낸다 </title>
				...
				$.ajax(
						{
							...
							success : function(data)
							{
								...
								$('#container').find("table tr").not(":first").remove();  //수업용이라서 더 복잡하다.  원래는 걍 덮어써도 됨
								
								$.each(data, function(){
									//console.log(this);
									$('#container').find("table tr:last").after(    //after 는 닫는태그에 붙이는거 
										"<tr>"
										+ " <td>" + this.seq + "</td>"
										+ " <td>" + this.comment + "</td>"
										+ " <td>삭제</td>"
