...
<head>
	...
	<title>데이터 조회 및 수정하기 data()함수 활용함 </title>
	<style type="text/css">
		.over {cursor:pointer;}
	</style>
	...
	<script type="text/javascript">
		...
		function RowUpdateBoard(){
			...
			$('#updateForm').on("submit",function(){
				var param = $(this).serialize(); //serialize() 파라메터를 일일이 만들지않아도 자동으로 만들어버림 , 네임속성을 가진 인풋태그를 파라메터로 만듦 
				//console.log(param);
				//alert("submit");
				$.ajax({
					...
					dataType:"JSON",
					type : "POST",
					data : param, //{"seq" :"1" ,"title" : "A" , "content":"A" , "regdate":"11" , "hit":"1"} ,  얘를 고댈로 들거와서 url 에 보내고 
					success : function(returndata){
							//JSON.parse(reutrndata);    계쏙 넘어오지 않으면 이렇게 해보세용
		...
		function GetBoardList(){
			...
			$.ajax({
				...
				success : function(data){
					...
					//data 함수를 안쓰면 쉬워진다. ...
					// 해쉬맵 만드는 것처럼 data 라는 함수를 사용하면 특정(지정한)태그에 키랑 밸류를 숨겨둘수있다????? 
					// 필요하면 끄내 쓸수있다!~
					
					//data -> [{},{},{},{}]   "list" 라는 이름을 지어주고, 가져온 json을 배열형태로 만들어주는것 
					...
					for(var k=0 ; k < data.length ; k++){
						...
						trObj.addClass("over").on("click",function(){    //over 라는 클래스스타일 붙여준당 그리고 클릭이벤트를 건다 
							//tr > click 하면 상세  출력 
							...
							//수정가능한 아래 테이블에 내용 보여주기
								...
								//https://www.w3schools.com/jquery/tryit.asp?filename=tryjquery_html_prop_attr    //이해 안되면 이거 보세용
						...
						$('#listView').append(trObj);  //리스트뷰에 어펜드까지~~!
					}
				}
			});
		}
