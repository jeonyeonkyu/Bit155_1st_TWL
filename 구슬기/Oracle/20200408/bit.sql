...
select * from v_001;  --조회가 된다 신기방기......      --view 는 sql 문장을 가지고 있는 객체다    ..얘를 실행하면 내부적으로는 윗 줄의 select empno, ename from emp 를 실행시킨다.
...
select * from v_003;    --부서별 평균 월급을 담고 있는 테이블이다.                                     얘는 등록된 코드고, in-line-view 는 그때그때 만드는 일회성 코드(쓰면 버리는~)이다.
...
select * from SYS.USER_SEQUENCES;    --(이름)BOARD_NUM 	(최소값)1	(최대값)9999999999999999999999999999	   (증가값)1 	N	N	20	1   
...
--19> 사원의 이름과 커미션을 출력하되, 커미션이 책정되지 않은     ***********************************decode , nvl 
